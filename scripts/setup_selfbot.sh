...
if ! id -u runewager >/dev/null 2>&1; then
  sudo useradd --system --home "$INSTALL_DIR" --shell /usr/sbin/nologin runewager
fi
sudo chown -R runewager:runewager "$INSTALL_DIR"
...
ExecStart=$INSTALL_DIR/.venv/bin/python -m qa_system.executor --service --root $INSTALL_DIR
User=runewager
Restart=always
RestartSec=5
