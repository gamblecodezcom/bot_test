...
def _validate_actions_payload(payload: Any) -> list[dict[str, Any]]:
    if not isinstance(payload, list):
        raise QueueValidationError("Queued action payload must be a JSON array")
    valid_actions: list[dict[str, Any]] = []
    for idx, action in enumerate(payload):
        if not isinstance(action, dict):
            raise QueueValidationError(f"Action at index {idx} must be an object")
        action_type = action.get("type")
        if not isinstance(action_type, str) or action_type not in {"send_command", "press_callback", "set_mode"}:
            raise QueueValidationError(f"Action at index {idx} has invalid type: {action_type}")
        raw_payload = action.get("payload", {})
        if not isinstance(raw_payload, dict):
            raise QueueValidationError(f"Action at index {idx} payload must be an object")
...
def queue_actions(root: Path, bot_name: str, action_file: Path) -> None:
    ...
    raw_actions = json.loads(action_file.read_text(encoding="utf-8"))
    actions = _validate_actions_payload(raw_actions)
    queue_path.parent.mkdir(parents=True, exist_ok=True)
    current = _read_json(queue_path, [])
    if not isinstance(current, list):
        current = []
    for action in actions:
        if not isinstance(action.get("timestamp"), str) or not action["timestamp"].strip():
