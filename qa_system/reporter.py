...
def write_logs(output_dir: Path, scenarios: list[Scenario], dry_run: bool) -> None:
    ...
    for scn in scenarios:
        status = "skipped_inactive" if not scn.active else ("simulated_pass" if dry_run else "pending_executor")
        action_log.append(
            {
                ...
                "role": scn.role,
                "status": status,
                "skipped_reason": scn.skipped_reason,
            }
        )
        ...
        message_log.append({"scenario_id": scn.scenario_id, "platform": scn.platform, "messages": ["placeholder: connector response capture"]})
