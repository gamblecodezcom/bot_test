...
class QAExecutor:
    ...
    def _apply_control_command(self, text: str) -> bool:
        ...
        if stripped == "/qa_on":
            ...
            if len(parts) == 2:
                ...
                try:
                    self.registry.select_bot(bot_name)
                except ValueError:
                    return False
                self.state.selected_bot = bot_name
            else:
                return False
        else:
            return False
    ...
    def _consume_actions(self) -> list[dict[str, Any]]:
        ...
        if not isinstance(parsed, list):
            self.write_log({"timestamp": datetime.now(timezone.utc).isoformat(), "error": "invalid_queue_format", "detail": "queue.json must be a list"}, "error_log.json")
            queue_file.write_text("[]", encoding="utf-8")
            return []
        queue_file.write_text("[]", encoding="utf-8")
        self._processed_actions += len(parsed)
        valid_actions = [a for a in parsed if isinstance(a, dict) and isinstance(a.get("type"), str) and isinstance(a.get("payload", {}), dict)]
        dropped = len(parsed) - len(valid_actions)
        if dropped:
            self.write_log({"timestamp": datetime.now(timezone.utc).isoformat(), "error": "invalid_actions_dropped", "count": dropped}, "error_log.json")
        return valid_actions

    def _replace_queue(self, actions: list[dict[str, Any]]) -> None:
        queue_file = self._bot_queue_file(self.state.selected_bot)
        queue_file.parent.mkdir(parents=True, exist_ok=True)
        queue_file.write_text(json.dumps(actions, indent=2), encoding="utf-8")
    ...
    async def run_service(self, poll_interval: float = 1.0) -> None:
        ...
        async with app:
            while True:
                ...
                passthrough_actions: list[dict[str, Any]] = []
                for action in actions:
                    ...
                    if action_type == "set_mode":
                        ...
                        if mode in {"user", "admin"}:
                            ...
                            self._save_state()
                            self.write_log({"timestamp": action.get("timestamp"), "action": "set_mode", "mode": mode})
                        continue
                    if action_type == "send_command":
                        text = str(payload.get("text", "")).strip()
                        if text and self._apply_control_command(text):
                            self.write_log({"timestamp": action.get("timestamp"), "action": text, "mode": self.state.mode})
                            continue
                    passthrough_actions.append(action)

                if not self.state.qa_enabled:
                    if passthrough_actions:
                        self._replace_queue(passthrough_actions)
                ...
                for action in passthrough_actions:
                    ...
                    if action_type == "send_command":
                        ...
                        self.write_log({"timestamp": action.get("timestamp"), "action": "send_command", "text": text, "message_id": sent.id, "mode": self.state.mode})
                    elif action_type == "press_callback":
                        self.write_log({"timestamp": action.get("timestamp"), "action": "press_callback", "payload": payload, "mode": self.state.mode})
                    else:
